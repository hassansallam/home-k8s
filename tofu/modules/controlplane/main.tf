...
resource "terraform_data" "cp_disk" {
  count = var.count_cp

  input = {
    vol_name  = "${var.name_prefix}-${count.index + 1}-disk.qcow2"
    disk_path = local.cp_disk_paths[count.index]
...
resource "libvirt_domain" "controlplane" {
  ...
  depends_on = [terraform_data.cp_disk]
    ...
    disks = [
      {
        ...
        source = {
          file = {
            file = local.cp_disk_paths[count.index]
          }
        }
        target = {
          dev = "vda"
